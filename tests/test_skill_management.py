...
# ===========================================================================
# 13. cmd_backup Tests — audit issues #5, #8
# ===========================================================================

class TestCmdBackup(unittest.TestCase):

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="skillsafe_backup_")
        self.skill_dir = Path(self.tmpdir) / "my-skill"
        self.skill_dir.mkdir()
        (self.skill_dir / "SKILL.md").write_text("# My Skill\n")
        (self.skill_dir / "main.py").write_text("x = 1\n")
    ...
    def test_backup_name_validated(self):
        """Audit #5 fix: --name flag is now validated against naming rules."""
        # This name has special chars that should be rejected
        args = _mock_args(path=str(self.skill_dir), name="../../evil", version="1.0.0")
        with self.assertRaises(SystemExit) as ctx:
            skillsafe.cmd_backup(args)
        self.assertEqual(ctx.exception.code, 1)
...
# ===========================================================================
# 14. cmd_restore Tests — audit issues #10, #12
# ===========================================================================

class TestCmdRestore(unittest.TestCase):

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="skillsafe_restore_")
        self._orig_config_dir = skillsafe.CONFIG_DIR
        self._orig_config_file = skillsafe.CONFIG_FILE
        self.config_dir = Path(self.tmpdir) / "config"
        self.config_dir.mkdir()
    ...
    def test_restore_path_traversal_rejected(self):
        args = _mock_args(name="../../../etc", output=str(Path(self.tmpdir) / "out"), skills_dir=None, tool=None)
        with self.assertRaises(SystemExit):
            skillsafe.cmd_restore(args)

    def test_restore_empty_name_rejected(self):
        args = _mock_args(name="", output=str(Path(self.tmpdir) / "out"), skills_dir=None, tool=None)
        with self.assertRaises(SystemExit):
            skillsafe.cmd_restore(args)
    ...
    def test_restore_with_verification(self, mock_meta, mock_download):
        ...
        with mock.patch.object(skillsafe.SkillSafeClient, "verify") as mock_verify:
            mock_verify.return_value = {"verdict": "verified", "details": {}}
            skillsafe.cmd_restore(args)
            # verify IS now called in restore
            mock_verify.assert_called_once()

    @mock.patch.object(skillsafe.SkillSafeClient, "get_metadata")
    def test_restore_no_versions_exits(self, mock_meta):
        mock_meta.return_value = {"latest_version": None}
        args = _mock_args(name="my-skill", output=str(Path(self.tmpdir) / "out"), skills_dir=None, tool=None)
        with self.assertRaises(SystemExit):
            skillsafe.cmd_restore(args)
